  ...
  async function fetchUserSeries() {
    ...
    try {
      ...
      animeList.value = data.userSeries.map((userSeries): Anime => {
        ...
        const hasPendingEpisode = pendingEpisodeIds.value.has(userSeries.series.anilistId)
        return {
          id: userSeries.series.anilistId,
          title: userSeries.series.romajiName,
          cover: userSeries.series.portraitUrl ?? '',
          progress: hasPendingEpisode && previous ? previous.progress : userSeries.lastEpisodeWatchedCount,
          ...
          isCompleted: hasPendingEpisode && previous ? previous.isCompleted : userSeries.isCompleted,
